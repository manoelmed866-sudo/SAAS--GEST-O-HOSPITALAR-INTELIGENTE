...
- Sprint 05 CONCLUIDA FUNCIONALMENTE na branch `sprint/05-cadastro-institucional`, pronta para merge posterior na `main`. Entrega: estrutura institucional do hospital configuravel por instituicao — unidades, setores, leitos e recursos institucionais — sob o modelo multi-tenant e as capacidades semanticas das Sprints 03/04. Cadastro de recurso NAO significa recurso utilizado; nenhuma tabela clinica foi criada.
- Migration `20260715020000_sprint_05_hospital_structure.sql` cria `hospital_units`, `hospital_sectors`, `hospital_beds` e `hospital_resources`, todas com `organization_id`/`hospital_id` obrigatorios, FK COMPOSTA para `hospitals(id, organization_id)` (isolamento por construcao), codigo unico por hospital, checks de formato, referencia publica opaca `management_ref` (padrao DEC-057), autoria (`created_by`) e timestamps com trigger. Hierarquia garantida no banco: setor pertence a unidade do MESMO hospital e leito a setor do MESMO hospital (FKs compostas) e triggers impedem criar filho sob pai inativo. Nenhum DELETE: desativacao logica por status.
- Novas permissoes semanticas `hospital_structure.read`/`hospital_structure.manage` (escopos organization e hospital): organization_admin e hospital_admin gerenciam; auditores leem; `member` nao acessa; `platform_admin` sem bypass. RLS fail-closed nas quatro tabelas com grants minimos (SELECT de tabela; INSERT por colunas de cadastro; UPDATE somente de `status`; `management_ref`/`created_by`/`status` inicial nao gravaveis pelo cliente; nenhuma policy para anon).
- `get_effective_hospital_capabilities` estendida (drop + recreate, SECURITY INVOKER preservado) com `can_read_structure` e `can_manage_structure`; resolver `resolveActiveHospitalCapabilities()` passa a validar SETE booleanos com Zod estrito. NENHUMA funcao SECURITY DEFINER nova foi criada na Sprint 05: leituras e mutacoes sao diretas sob o cliente autenticado, com o RLS como barreira final (DEC-059).
- Aplicacao: resolver `resolveActiveHospitalStructure()` (gate por `canReadStructure`, hospital exclusivamente do contexto ativo revalidado, aninhamento unidade -> setor -> leito montado no servidor, UUIDs internos descartados, referencias opacas somente para quem gerencia, fail-closed); Server Actions de criacao (unidade, setor, leito, recurso) e de ativacao/desativacao com Zod estrito, enum fechado de tipos, pai resolvido no servidor por referencia opaca restrita ao hospital ativo e traducao amigavel de erros (codigo duplicado, pai indisponivel, negado); pagina `/painel/admin/estrutura` (Server Component `force-dynamic`) com estados allowed/vazio/denied/absent/invalid/error, formularios e controles somente para gestores e confirmacao explicita inline; painel com link "Estrutura do hospital" condicionado somente a `canReadStructure`.
- Testes: 508 unitarios aprovados (novas suites de resolver, actions, pagina, componentes e revisao estatica `sprint-05-static-security`; suites 04A/04B e `auth-capabilities` atualizadas legitimamente para sete capacidades) e 341 verificacoes pgTAP aprovadas (novo `011-sprint-05-hospital-structure.test.sql` com 76 verificacoes: estrutura, RLS, grants, isolamento entre hospitais e organizacoes, perfis autorizados/negados, invariantes e capacidades; teste 001 atualizado legitimamente para o escopo da Sprint 05).
- E2E aprovado em NAVEGADOR REAL (Chromium headless via CDP, sem dependencia nova) contra o build de PRODUCAO local com hidratacao real: 28 verificacoes — admin cria unidade/setor/leito/recurso pela interface, codigo duplicado bloqueado, FormData minimo (somente `kind` + referencia opaca de 32 hex + `requestedStatus`), cancelamento sem mutacao, desativacao confirmada; auditor somente leitura sem formularios/controles/referencias; member sem link e negado por URL direta; isolamento entre organizacoes comprovado; rota reprotegida apos logout. Conferencia no banco exata e fixtures integralmente removidas com zero orfaos.
- Aprovados lint, typecheck, 508 testes unitarios, build, `db:lint` e 341 verificacoes pgTAP. Decisao registrada em `DECISIONS.md` como DEC-059. Sprint 06 NAO foi iniciada.

### Fechamento - Sprint 04 - Gestao de papeis hospitalares e conclusao funcional

- Sprint 04 CONCLUIDA FUNCIONALMENTE na branch `sprint/04-administracao-governanca`, pronta para merge posterior na `main`. Entrega final: gestao de papeis hospitalares existentes (atribuir, revogar e reatribuir) por RPC transacional auditada, fechando o ultimo caminho de mutacao direta da administracao da equipe.
- Referencia publica opaca `roles.management_ref` (128 bits, 32 hex, `gen_random_bytes`, unique + check): nenhum id interno de papel, `role.code` ou `permission.code` trafega em HTML ou FormData; a referencia nao autoriza nada.
- Hardening RPC-only de `hospital_membership_roles`: revogados INSERT/UPDATE/DELETE (tabela e colunas) de `authenticated` e removidas as policies `hospital_membership_roles_insert_allowed` e `hospital_membership_roles_update_allowed`; SELECT legitimo preservado. `organization_membership_roles`, `platform_role_assignments` e os catalogos `roles`/`permissions` permanecem intocados.
- Auditoria administrativa ESTENDIDA (sem tabela paralela): novos eventos `hospital_role_assigned` e `hospital_role_revoked`, coluna opcional `target_role_id` (FK para roles) e constraint cruzada ampliada — eventos de vinculo nao carregam papel; eventos de papel exigem o papel e somente as transicoes coerentes (`none|revoked -> active`; `active -> revoked`). Append-only, RLS fechado, zero acesso direto da aplicacao, insercao na MESMA transacao da mutacao.
- RPC unica `public.change_hospital_membership_role(uuid, text, text, text)`, `plpgsql`, `volatile`, **SECURITY DEFINER**, `set search_path = ''`: acoes `assign`/`revoke`; autorizacao fail-closed por `hospital_memberships.manage` (escopo hospitalar OU organizacional, sem bypass de platform_admin); lock por hospital; alvo e papel por referencias opacas sem enumeracao; papel restrito ao scope hospital; reatribuicao reativa a linha revogada existente (sem duplicata, unique por vinculo/papel); auto-revogacao do proprio `hospital_admin` bloqueada (`self_admin_role_forbidden`); ultimo `hospital_admin` ativo qualificado protegido (`last_admin_forbidden`, apos o lock). `EXECUTE` somente para `authenticated`.
- Nova RPC de leitura `public.get_hospital_assignable_roles(uuid)` (SECURITY DEFINER restrito, manage-only): catalogo hospitalar minimo atribuivel com rotulo amigavel e referencia opaca. `get_hospital_team` estendida com `assigned_roles` (label, roleRef, canRevoke) somente para quem gerencia; auditor continua sem referencias e sem controles.
- Aplicacao: Server Action `changeMembershipRoleAction` (Zod estrito, apenas `membershipRef`+`roleRef`+`requestedAction`, hospital exclusivamente do contexto ativo, cinco outcomes mapeados, revalidacao somente em sucesso) e componente cliente `TeamRoleControls` (select do catalogo, confirmacao explicita inline, cancelamento sem mutacao). Nenhuma criacao de papel, edicao de permissao, papel organizacional ou de plataforma.
- DIFERIDO com registro em decisao: administracao de identidade (convites, criacao de contas, recuperacao de senha) exige Supabase Admin API/service_role e arquitetura privilegiada propria — fica para Sprint dedicada; vinculo de perfil ja existente ao hospital foi diferido por ausencia de mecanismo seguro de descoberta de perfis sob RLS (sem diretorio global de usuarios). Itens de governanca visual (painel de auditoria, workspaces por perfil, design system autenticado) permanecem como trilha futura e nao bloqueiam as sprints clinicas.
- Testes: `supabase/tests/010-sprint-04-role-management.test.sql` (67 pgTAP: refs opacas, hardening, auditoria estendida, RPC, isolamentos, assign/revoke/reatribuicao, invariantes e catalogo); pgTAP 008 atualizado para a nova assinatura; novos `team-role-actions.test.ts`, `team-role-controls.test.tsx` e `sprint-04-role-static-security.test.ts`; suites existentes atualizadas legitimamente.
- E2E aprovado em NAVEGADOR REAL (Chromium headless via CDP, sem dependencia nova), 26 verificacoes: member sem acesso; auditor com lista e papeis, sem nenhum controle ou referencia; admin atribui e revoga papel com confirmacao, cancelamento sem mutacao e auto-revogacao indisponivel; organization_admin revoga um de dois admins, ultimo admin protegido tambem na interface e estado restaurado; FormData minimo; INSERT/UPDATE diretos via PostgREST negados (HTTP 403). Auditoria conferida: 4 eventos exatos (2 atribuicoes, 2 revogacoes; reatribuicao com previous `revoked`), zero eventos por cancelamento/falha, zero combinacoes inconsistentes, zero duplicatas. Fixtures integralmente removidas com contagens zeradas.
- Aprovados lint, typecheck, 406 testes unitarios, build, `db:lint` e 265 verificacoes pgTAP.
- Decisao registrada em `DECISIONS.md` como DEC-058. Sprint 05 NAO foi iniciada.

## 2026-07-14

### Checkpoint - Sprint 04C.2 - Suspensao e reativacao de vinculos com auditoria

- Sprint 04C.2 concluida na branch `sprint/04-administracao-governanca`: primeiras mutacoes administrativas reais, restritas a suspensao (`active -> suspended`) e reativacao (`suspended -> active`) de vinculos hospitalares, com trilha de auditoria transacional. Nenhuma exclusao, revogacao, alteracao de papel, convite ou criacao de conta; `pending` e `revoked` ficam fora do escopo (`revoked` permanece terminal).
- Criada a referencia publica opaca `hospital_memberships.management_ref` (128 bits, 32 hex, `gen_random_bytes`, unique + check de formato): o UUID interno do vinculo nunca trafega em HTML ou FormData. A referencia opaca NAO autoriza nada; e apenas um endereco de alvo, e a RPC revalida toda a autorizacao internamente.
- Criada a tabela append-only `public.administrative_audit_events` com RLS habilitado e ZERO grants/policies para a aplicacao: nenhuma leitura ou escrita direta; a insercao ocorre exclusivamente dentro da RPC de mutacao, na MESMA transacao da alteracao (falha reverte ambas). Constraint cruzada `administrative_audit_events_transition_consistency_check` aceita somente as combinacoes coerentes evento/transicao (suspended: active->suspended; reactivated: suspended->active).
- Criada a RPC unica de mutacao `public.change_hospital_membership_status(uuid, text, text)`, `plpgsql`, `volatile`, **SECURITY DEFINER** com `set search_path = ''`: autorizacao explicita fail-closed (perfil ativo + `hospital_memberships.manage` por escopo hospitalar OU organizacional, sem bypass de platform_admin), lock por hospital (`FOR UPDATE`) serializando mutacoes concorrentes, alvo resolvido pela referencia opaca restrita ao hospital travado (alvo inexistente e fora de escopo retornam o mesmo resultado, sem enumeracao), auto-suspensao bloqueada (`self_suspension_forbidden`) e ultimo administrador ativo protegido (`last_admin_forbidden`, checado APOS o lock). `EXECUTE` somente para `authenticated`.
- Hardening RPC-only (migration complementar `20260714030000`): revogado o `UPDATE (status)` direto de `authenticated` em `hospital_memberships` (concedido na Sprint 03A) e removida a policy `hospital_memberships_update_allowed`; a RPC passou a ser o UNICO caminho de alteracao de status de vinculo hospitalar. Nenhum grant novo, nenhuma policy de leitura alterada, nenhuma mudanca em `organization_memberships` ou nas tabelas de papeis.
- `get_hospital_team` estendida com metadados de acao apenas para quem possui manage (`management_ref`, `can_suspend`, `can_reactivate`); leitores sem manage (auditor) recebem referencia nula e indicadores falsos. Indicadores sao orientacao de interface; a RPC de mutacao revalida tudo.
- Server Action `changeMembershipStatusAction` (`actions.ts`): recebe SOMENTE `managementRef` e `requestedStatus`, valida com Zod, exige `canManageMemberships` e delega a RPC com o hospital vindo exclusivamente do contexto ativo revalidado; mensagens genericas sem UUID ou codigo cru. Componente cliente `TeamMemberControls` com confirmacao explicita inline (Confirmar suspensao/reativacao + Cancelar) antes de qualquer submissao.
- Adicionados/atualizados testes: `supabase/tests/009-sprint-04c2-membership-mutations.test.sql` (59 pgTAP: estrutura, grants, hardening, autorizacao, isolamento, transicoes, invariantes, auditoria e metadados), `tests/unit/team-membership-actions.test.ts`, `tests/unit/team-member-controls.test.tsx` e atualizacoes em `auth-hospital-team`, `auth-admin-team-page` e testes estaticos.
- E2E aprovado em NAVEGADOR REAL (Chromium headless via CDP, JavaScript e hidratacao reais, sem dependencia nova), 35 verificacoes: member negado sem lista/botoes/dados internos; auditor com lista e sem nenhum controle, referencia opaca ou formulario; admin com acoes coerentes por estado (pending sem acao, self sem suspensao); FormData contendo somente `managementRef` + `requestedStatus`; cancelamento sem mutacao; suspensao e reativacao com mensagem, status e botao atualizados; ultimo administrador protegido tambem na interface; UPDATE direto via PostgREST negado (HTTP 403); logout e reprotecao de `/painel`. Auditoria conferida no banco: exatamente 1 evento por mutacao bem-sucedida (4 no total), ator/hospital/alvo/estados/timestamp corretos, zero eventos por cancelamento ou falha, zero combinacoes inconsistentes. Lock comprovado estruturalmente (sem teste real de concorrencia). Fixtures integralmente removidas com contagens zeradas.
- Aprovados lint, typecheck, 368 testes unitarios, build, `db:lint` e 198 verificacoes pgTAP.
- Decisao registrada em `DECISIONS.md` como DEC-057. Gestao de papeis, convites e demais mutacoes pertencem a 04C.3+, nao iniciada.
